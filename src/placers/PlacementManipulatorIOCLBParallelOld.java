...
public class PlacementManipulatorIOCLBParallelOld implements PlacementManipulator {
	...
	public PlacementManipulatorIOCLBParallelOld(FourLutSanitized a, Circuit c) {
		this.a=a;
		this.circuit=c;
	...
	public PlacementManipulatorIOCLBParallelOld(FourLutSanitized a, Circuit c, Random rand) {
		this.a=a;
		this.circuit=c;
		maxFPGAdimension = Math.max(a.width,a.height);
		circuit.vBlocks = new Vector<Block>();
	...
	public Swap findIndependentSwap(int Rlim) {
		//the source code in this function needs to be changed
		Swap swap=new Swap();
		Block b = circuit.vBlocks.elementAt(rand.nextInt(circuit.vBlocks.size()));
		swap.pl1 = b.site;
		if(b.type==BlockType.CLB){
